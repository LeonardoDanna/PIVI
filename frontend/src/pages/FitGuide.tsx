...
// Ícone customizado de Calça com tipagem
const PantsIcon = ({ size = 24, className = "" }: IconProps) => (
  <svg
...
const FitGuide = () => {
  const [activeView, setActiveView] = useState<"calculator" | "table">(
    "calculator"
  );
  ...
  const handleCopy = () => {
    ...
    // Correção: Usando a API moderna de Clipboard em vez de execCommand
          ...
          {activeView === "calculator" ? (
            <div className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end">
                <button
                  onClick={handleReset}
                  className="text-xs font-bold text-slate-400 hover:text-red-500 flex items-center gap-1 transition-colors px-2 py-1 rounded hover:bg-red-50"
                >
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 h-full overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-slate-50 text-slate-500 uppercase font-bold text-xs sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 rounded-tl-lg border-b border-slate-200">
                      Tam
                    </th>
                    <th className="px-4 py-3 border-b border-slate-200">
                      Altura
                    </th>
                    <th className="px-4 py-3 rounded-tr-lg border-b border-slate-200">
                      Peso
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {REFERENCE_TABLE.map((row) => {
                    const isActive =
                      calculatedSize.top === row.size.split(" / ")[0] ||
                      calculatedSize.bottom === row.size.split(" / ")[1];
                    return (
                      <tr
                        key={row.size}
                      ...
                      >
                        <td className="px-4 py-3 font-bold text-slate-800">
                          {row.size}
                          {isActive && (
                            <span className="ml-2 text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full align-middle">
                              SEU
                            </span>
                          )}
                        </td>
      ...
      <div className="col-span-1 lg:col-span-7 flex flex-col gap-6">
        <div className="bg-slate-900 rounded-3xl p-6 sm:p-8 text-white relative flex-1 flex flex-col items-center text-center shadow-xl min-h-[360px] group/card overflow-hidden">
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div
              className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] opacity-40 -mr-20 -mt-20 transition-colors duration-500 ${
                resultType === "top" ? "bg-blue-600" : "bg-indigo-600"
              }`}
            ></div>
            <div
              className={`absolute bottom-0 left-0 w-64 h-64 rounded-full blur-[80px] opacity-40 -ml-20 -mb-20 transition-colors duration-500 ${
                resultType === "top" ? "bg-purple-600" : "bg-teal-600"
              }`}
            ></div>
          </div>
          ...
          <div className="relative z-20 w-full flex justify-center mb-6">
            <div className="bg-slate-800/50 p-1.5 rounded-2xl flex gap-1 backdrop-blur-md border border-white/10 shadow-lg">
              ...
              >
                <Shirt
                  size={18}
                  className={resultType === "top" ? "text-blue-600" : ""}
                />
                Superior
              </button>
              <button
              ...
              >
                <PantsIcon
                  size={18}
                  className={resultType === "bottom" ? "text-indigo-600" : ""}
                />
                Inferior
              </button>
            </div>
