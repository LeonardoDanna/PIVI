...
import { 
  Star, User, Camera, Upload, X, CheckCircle, 
  Palette, Ruler, Sparkles, AlertCircle, ChevronRight, Share2, 
  Clock, TrendingUp
} from 'lucide-react';
...
const StylistAI = () => {
  ...
  const startAnalysis = () => {
    ...
    // Simulate AI processing time & detailed result generation
    setTimeout(() => {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-8 font-sans">
      <div className="max-w-6xl w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden p-6 md:p-12 min-h-[700px] flex flex-col">
        ...
        <nav className="flex justify-between items-center mb-8 md:mb-12 shrink-0">
          <div className="text-xl font-bold tracking-tighter text-slate-900 flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('stylist')}>
            <Sparkles className="text-purple-600" size={24} />
            <span>FASHION<span className="text-purple-600">AI</span></span>
          </div>
          <div className="hidden md:flex gap-2 text-sm font-medium text-slate-500">
            <button 
              onClick={() => setActiveTab('history')}
              className={`px-4 py-2 rounded-full transition-all ${activeTab === 'history' ? 'bg-slate-900 text-white' : 'hover:bg-slate-100 hover:text-slate-900'}`}
            >
              Histórico
            </button>
            <button 
              onClick={() => setActiveTab('trends')}
              className={`px-4 py-2 rounded-full transition-all ${activeTab === 'trends' ? 'bg-slate-900 text-white' : 'hover:bg-slate-100 hover:text-slate-900'}`}
            >
              Tendências
            </button>
            <button 
              onClick={() => setActiveTab('stylist')}
              className={`px-4 py-2 rounded-full transition-all ${activeTab === 'stylist' ? 'bg-slate-900 text-white shadow-lg shadow-purple-200/50' : 'hover:bg-slate-100 hover:text-slate-900'}`}
            >
              AI Stylist
            </button>
          </div>
        ...
        <div className="flex-grow">
          {activeTab === 'stylist' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start h-full animate-fade-in">
              ...
              <div className="space-y-8 pt-4">
                <div>
                  ...
                  <p className="text-lg text-slate-600 leading-relaxed">
                    Faça o upload do seu look atual. Nossa IA analisa proporções, 
                    detecta sua paleta e sugere <span className="font-bold text-slate-800">melhorias reais</span> para elevar sua imagem.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="group hover:bg-slate-50 transition-all duration-300 flex items-start gap-5 p-5 bg-white rounded-2xl border border-slate-100 shadow-sm cursor-default">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-slate-800">Análise Biométrica</h4>
                  ...
                  <div className="group hover:bg-slate-50 transition-all duration-300 flex items-start gap-5 p-5 bg-white rounded-2xl border border-slate-100 shadow-sm cursor-default">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-slate-800">Psicologia das Cores</h4>
                      <p className="text-slate-500 text-sm">
                ...
                <div className="pt-4">
                  <button
                    ...
                    className={`
                      w-full md:w-auto px-8 py-4 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 transition-all duration-300 shadow-xl shadow-purple-100
                      ${!image 
                  ...
                  >
                    {isAnalyzing ? (
                      ...
                      <>
                        <Camera size={22} /> 
                        {analysisComplete ? 'Analisar Novo Look' : 'Iniciar Análise Completa'}
                      </>
                    )}
                  </button>
                </div>
              ...
              {/* Right Column: Upload Area & Result */}
              <div className="relative h-[650px] w-full group">
                ...
                <div 
                  className={`
                    relative z-10 h-full bg-white rounded-[2rem] flex flex-col items-center justify-center border-2 border-dashed transition-all duration-500 overflow-hidden shadow-sm
                    ${image ? 'border-solid border-white p-0 shadow-2xl ring-4 ring-purple-50/50' : 'border-slate-300 hover:border-purple-400 hover:bg-purple-50/30'}
                  `}
                ...
                >
                  {!image ? (
                    ...
                    >
                      ...
                      <h3 className="text-2xl font-bold text-slate-800 mb-2">Upload do Look</h3>
                      <p className="text-slate-500 mb-8 max-w-xs mx-auto text-sm leading-relaxed">
                        Arraste sua foto de corpo inteiro para cá ou clique para explorar seus arquivos.
                      </p>
                    ...
                    <>
                      <div className="relative w-full h-full bg-slate-100">
                        ...
                        {analysisComplete && <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/20 to-transparent transition-opacity duration-700" />}
                      </div>
                      ...
                      {isAnalyzing && (
                        <div className="absolute inset-0 bg-slate-900/40 z-20 flex flex-col items-center justify-center backdrop-blur-sm">
                          ...
                          <div className="bg-white/10 backdrop-blur-md border border-white/20 px-8 py-6 rounded-2xl shadow-2xl flex flex-col items-center gap-3">
                            ...
                            <p className="font-bold text-white text-lg tracking-wide">Mapeando Pontos...</p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="absolute bottom-0 inset-x-0 bg-white rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.2)] z-20 animate-slide-up max-h-[85%] overflow-y-auto custom-scrollbar">
                          ...
                          <div className="p-6 md:p-8 space-y-6">
                            ...
                            <div className="flex items-start justify-between border-b border-slate-100 pb-6">
                              <div>
                                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Veredito da IA</div>
                                <h3 className="text-2xl font-black text-slate-900 mb-1">{resultData.styleClass}</h3>
                                <div className="flex items-center gap-2 text-green-600 text-sm font-semibold">
                                  <CheckCircle size={16} /> Look Aprovado
                                </div>
                              </div>
                              <div className="flex flex-col items-end">
                                <div className="text-4xl font-black text-purple-600 tracking-tighter">{resultData.score}</div>
                            ...
                            <div>
                              <h4 className="font-bold text-slate-800 mb-3 text-sm flex items-center gap-2">
                                <Palette size={16} className="text-purple-500" /> Paleta Detectada
                              </h4>
                              <div className="flex gap-3">
                                {resultData.palette.map((color, idx) => (
                                  <div key={idx} className="flex flex-col items-center gap-1 group cursor-pointer">
                                    ...
                                    />
                                    <span className="text-[10px] font-medium text-slate-500 group-hover:text-purple-600">{color.name}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                              ...
                              <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
                                ...
                                <ul className="space-y-2">
                                  {resultData.strengths.map((item, i) => (
                                    <li key={i} className="text-sm text-green-700 leading-snug flex items-start gap-2">
                              ...
                              <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                                ...
                                <ul className="space-y-2">
                                  {resultData.improvements.map((item, i) => (
                                    <li key={i} className="text-sm text-orange-700 leading-snug flex items-start gap-2">
          ...
          {activeTab === 'history' && (
            <div className="h-full flex flex-col items-center justify-center animate-fade-in text-slate-500 py-20">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Seu Histórico</h3>
              <p className="max-w-md text-center mb-8">
                Aqui você encontrará todas as suas análises anteriores. Faça sua primeira análise na aba "AI Stylist" para começar.
              </p>
              <button onClick={() => setActiveTab('stylist')} className="text-purple-600 font-bold hover:underline">
                Voltar para Análise
              </button>
            </div>
          )}

          {/* Placeholder for Tendências */}
          {activeTab === 'trends' && (
            <div className="h-full flex flex-col items-center justify-center animate-fade-in text-slate-500 py-20">
               <div className="bg-slate-100 p-6 rounded-full mb-4">
                <TrendingUp size={48} className="text-slate-300" />
              </div>
              <h3 className="text-xl font-bold text-slate-900 mb-2">Tendências Globais</h3>
              <p className="max-w-md text-center mb-8">
                Nossa IA está escaneando as redes sociais e desfiles. As tendências da estação aparecerão aqui em breve.
              </p>
              <button onClick={() => setActiveTab('stylist')} className="text-purple-600 font-bold hover:underline">
                Voltar para Análise
              </button>
